...
const MusicModal: React.FC<MusicModalProps> = ({ track, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-800 rounded-lg shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col md:flex-row overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="w-full md:w-1/2 aspect-video md:aspect-auto bg-black">
          <iframe
            width="100%"
            height="100%"
        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col overflow-y-auto">
          <div className="flex-grow">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">{track.description}</p>
            
            <div className="mb-6 space-y-3">
                <h4 className="font-bold text-white">Listen On</h4>
                <div className="flex space-x-3">
                    <a href={track.streamingLinks.spotify} target="_blank" rel="noopener noreferrer" className="flex-1 text-center bg-[#1DB954] text-white font-semibold py-2 px-4 rounded-md hover:bg-opacity-80 transition-opacity">Spotify</a>
                    <a href={track.streamingLinks.appleMusic} target="_blank" rel="noopener noreferrer" className="flex-1 text-center bg-white text-black font-semibold py-2 px-4 rounded-md hover:bg-opacity-80 transition-opacity">Apple Music</a>
                </div>
            </div>
